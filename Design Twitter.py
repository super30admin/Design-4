# Time Complexity: O(N) for getNewsFeed, O(1) for all other operations
# Space Complexity: O(number of tweets + number of users)

# Code ran on LeetCode
# Maintain a hash set to store tweets of all users and the tweet time stamp. Another hash set to store all the users followed by the given user. Use a min heap of size 10 to get the top 10 news feed based on time stamp.
import heapq
class Twitter:
