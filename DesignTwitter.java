...
// We use a hashmap of (integer, hashset<integer>) to store (users, following) mapping
// We use a hasmap of (integer, list<integer>) to store (users, tweets) mapping 
// Hashmaps are used so that we get O(1) lookup time
